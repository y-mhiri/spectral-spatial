...
class PANTVCtv(PANProximalGradient):
    ...
    def project_dual_ball_r(self, U, p_star, q_star, r_star, eps=1e-8):
        ...
        if torch.isinf(torch.tensor(p_star, device=U.device)):
            ...
            norm_p_star = torch.sum(torch.abs(U)**p_star, dim=1, keepdims=True)**(1/(p_star + eps))

        # Étape 2: Norme q* sur les dérivées (axis=-1)
        if torch.isinf(torch.tensor(q_star, device=U.device)):
            norm_q_star= torch.amax(norm_p_star, dim=-1, keepdim=True)   # l^infini
        else:
            norm_q_star = torch.sum(norm_p_star**q_star, dim=-1, keepdims=True)**(1/(q_star + eps))

        # Étape 3: Norme r* sur les pixels (axis=(2,3))
        if torch.isinf(torch.tensor(r_star, device=U.device)):
            norm_r_star= torch.amax(norm_q_star, dim=(2,3), keepdim=True)  # l^infini
        else:
            norm_r_star = torch.sum(norm_q_star**r_star, dim=(2,3), keepdims=True)**(1/(r_star + eps))
