...
def main():
    ...
    parser.add_argument("--image_idx", nargs="+", type=int, required=True)
    ...
    print_experiment_info(args, 'PANTVGradAlignment')
    
    # Create output directory and zarr file
    os.makedirs(args.storage_path, exist_ok=True)
    root = zarr.open(f'{args.storage_path}/results.zarr', mode='w')
    
    # Setup parameters with gradient alignment specific additions
    ...
    # Save results
    store_results(root, args, reconstructed_ar, loss_ar, metrics, total_time, 'PANTVGradAlignment')
    save_experiment_info(args.storage_path, args, total_time, metrics, 'PANTVGradAlignment')
