...
for i, (lmbda, mu) in enumerate(zip(root.attrs['lambda_range'], root.attrs['mu_range'])):
    print(f"Running trial {i} : lambda = {lmbda}, mu = {mu}")
    print('-----------------------------------')  
    group = root.create_group(f'trial_{i:02d}')
